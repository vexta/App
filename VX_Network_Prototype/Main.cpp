...
GLuint floorVAO, cubeVAO, cubeVBO, kinectMeshVAO, kinectMeshVBO, kinectMeshRecievedVAO, kinectMeshRecievedVBO;
...
Viewer viewer;
sceneObject object, kinectMesh, kinectMeshRecieved, headPos, leftHandPos, rightHandPos, cube;
...
int main() {
	...
	while (!ovrHmdHandle->shouldClose()) {
		...
		if (1 == 1) {	// if mesh data successfully retrieved
			...
			kinectMeshRecieved.model = glm::translate(glm::rotate(glm::mat4(1), 180.0f, glm::vec3(1, 0, 0)), glm::vec3(0.0, -2.0, 0.0));
