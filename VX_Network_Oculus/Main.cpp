...
int main() {
	...
	while (!ovrHmdHandle->shouldClose()) {
		...
		//gedy sa ma vygenerovat random
		//ci sa posunula kocka za treshold
		//vyfarbit kocku farbou
		//resetovat poziciu kociek
		...
		if (data.meshData) {	// if mesh data successfully retrieved
			...
			kinectMesh.model = glm::translate(glm::rotate(glm::mat4(1), 180.0f, glm::vec3(1, 0, 0)), glm::vec3(0.0, -4.0, 0.0));
		...
		//!!!! Poslat kocky s random a prijat kinect mesh
		//komunikacia
		if (1 == 1) {	// if mesh data successfully retrieved
			const Vector3 *vertices = nullptr, *normals = nullptr;
			//data.meshData->GetVertices(&vertices);
			//data.meshData->GetNormals(&normals);
			...
			kinectMeshRecieved.verticesCnt = recievedVertexCount;
			kinectMeshRecieved.model = glm::translate(glm::rotate(glm::mat4(1), 180.0f, glm::vec3(1, 0, 0)), glm::vec3(0, 1.0, 2.5));

			glBindVertexArray(kinectMeshRecievedVAO);
			glBindBuffer(GL_ARRAY_BUFFER, kinectMeshRecievedVBO);
